...
from modelexp import tanefexp, exputils, fetelexp
...
def train_nefaaa():
    ...
    results_file = os.path.join(config.RESULT_DIR, 'tanef-deep-results-{}.txt'.format(dataset))
    # noel_preds_file = os.path.join(config.RESULT_DIR, 'aaa-token-dhl-results-{}.txt'.format(dataset))
    noel_preds_file = os.path.join(config.DATA_DIR, 'Wiki/noel-fet-results-aaa-{}.txt'.format('figer'))
    ...
    # el_candidates_file = config.EL_CANDIDATES_DATA_WITHNIL_FILE
