...
from models.nefaaa import NEFAAA, TANEFAtt, NEFAAATest, NEFAAATestStack
from models.feteldeep import FETELStack
from models.fetentvecutils import FETEntityVec, ELDirectEntityVec
...
def eval_model(device, gres: exputils.GlobalRes, el_entityvec: FETEntityVec, use_attention, use_mlp,
               params_file, test_mentions_file, test_sents_file, noel_preds_file, output_results_file):
    logging.info('load model from {}'.format(params_file))
    ...
    logging.info('context_lstm_hidden_dim={} type_embed_dim={}'.format(context_lstm_hidden_dim, type_embed_dim))
    if use_attention:
        model = TANEFAtt(device, gres.type_vocab, gres.type_id_dict, gres.embedding_layer,
                         context_lstm_hidden_dim, type_embed_dim)
    else:
        model = NEFAAA(device, gres.type_vocab, gres.type_id_dict, gres.embedding_layer,
                       context_lstm_hidden_dim, type_embed_dim, use_mlp=use_mlp)
    if device.type == 'cuda':
        model = model.cuda(device.index)
    model.load_state_dict(trained_params, strict=True)

    samples = model_samples_from_json(gres.token_id_dict, gres.unknown_token_id, gres.mention_token_id,
                                      gres.type_id_dict, test_mentions_file, test_sents_file)
    true_labels_dict = {s.mention_id: [gres.type_vocab[l] for l in s.labels] for s in samples}
    noel_preds = datautils.read_pred_results_file(noel_preds_file)
    entity_vecs, el_sgns, el_probs = __get_entity_vecs_for_samples(el_entityvec, samples, noel_preds)
    _, sacc, pacc, _, _, result_objs = eval_nefaaa(gres, model, samples, true_labels_dict, entity_vecs, el_sgns,
                                                   el_probs, use_entity_vecs=True, single_type_path=False)
    # for robj in result_objs:
    #     print(max(robj['logits']), robj['logits'][gres.type_id_dict['/person/politician']])
    print(sacc)
    datautils.save_json_objs(result_objs, output_results_file)


def __get_tanefatt_model(device, gres, aaa_model_file, context_lstm_hidden_dim, type_embed_dim):
    if aaa_model_file is not None:
        trained_params = torch.load(aaa_model_file)
        context_lstm_hidden_dim = trained_params['context_lstm.weight_hh_l0'].size()[1]
        type_embed_dim = trained_params['type_embeddings'].size()[0]
        model = TANEFAtt(device, gres.type_vocab, gres.type_id_dict, gres.embedding_layer,
                         context_lstm_hidden_dim, type_embed_dim)
        model.load_state_dict(trained_params, strict=False)
        logging.info('model inited from {}'.format(aaa_model_file))
        return model
    return TANEFAtt(device, gres.type_vocab, gres.type_id_dict, gres.embedding_layer,
                    context_lstm_hidden_dim, type_embed_dim)
...
def train_nefaaa(device, gres: exputils.GlobalRes, el_entityvec: ELDirectEntityVec,
    ...
    try:
        ...
        # elif test and stack_lstm:
        # model = NEFAAATestStack(
        #     device, gres.type_vocab, gres.type_id_dict, gres.embedding_layer, context_lstm_hidden_dim,
        #     type_embed_dim=type_embed_dim, dropout=dropout, use_mlp=use_mlp, mlp_hidden_dim=pred_mlp_hdim,
...
#                dev_samples: List[ModelSample], test_mentions_file, test_sents_file, test_noel_preds_file, nil_rate,
#                learning_rate, n_iter, rand_per, per_penalty, single_type_path,
#                save_model_file=None, eval_batch_size=32, results_file=None):
#     lr_gamma = 0.7
#     logging.info('{}'.format(model.__class__.__name__))
#     dev_true_labels_dict = {s.mention_id: [gres.type_vocab[l] for l in s.labels] for s in dev_samples}
#     dev_entity_vecs, dev_el_sgns, dev_el_probs = __get_entity_vecs_for_samples(
...
#         logits = model(context_token_seqs, mention_token_idxs, mstr_token_seqs, entity_vecs, el_sgns, el_probs)
#         # elif use_attention:
#         #     logits, _, _ = model(context_token_seqs, mention_token_idxs, mstr_token_seqs, entity_vecs,
#         #                          el_sgns, el_probs)
#         # else:
#         #     logits = model(True, context_token_seqs, mention_token_idxs, mstr_token_seqs, entity_vecs, el_sgns)
#         loss = model.get_loss(type_vecs, logits, person_loss_vec=person_loss_vec)
#         scheduler.step()
#         optimizer.zero_grad()
#         loss.backward()
...
def eval_nefaaa(gres: exputils.GlobalRes, model, samples: List[ModelSample], true_labels_dict,
                entity_vecs, el_sgns, el_probs, batch_size=32, use_entity_vecs=True, use_attention=False,
                single_type_path=False, test=True):
    model.eval()
    n_batches = (len(samples) + batch_size - 1) // batch_size
    losses = list()
    ...
    for i in range(n_batches):
        ...
        with torch.no_grad():
            # logits = model(False, context_token_seqs, seq_lens, mention_token_idxs, mstrs, mstr_token_seqs)
            if test:
                logits = model(context_token_seqs, mention_token_idxs, mstr_token_seqs,
                               entity_vecs_batch, el_sgns_batch, el_probs_batch)
            elif use_attention:
                logits, att_vecs, channel_logits = model(context_token_seqs, mention_token_idxs, mstr_token_seqs,
                                                         entity_vecs_batch, el_sgns_batch, el_probs_batch)
            else:
                logits = model(False, context_token_seqs, mention_token_idxs, mstr_token_seqs,
                               entity_vecs_batch, el_sgns_batch)
        ...
        # preds = model.inference(logits)
        # tmp_el_type_vecs = entity_vecs[batch_beg:batch_end]
        # tmp_el_sgns = el_sgns[batch_beg:batch_end]
        channel_logits = [tmp.data.cpu().numpy() for tmp in channel_logits] if use_attention else None
        att_vecs = att_vecs.data.cpu().numpy() if use_attention else None
        for j, (sample, type_ids_pred, sample_logits) in enumerate(
                zip(batch_samples, preds, logits.data.cpu().numpy())):
            # pred_labels_dict[sample.mention_id] = utils.get_full_type_ids(type_ids_pred, gres.parent_type_ids_dict)
            att_vec = att_vecs[j] if use_attention else []
            cur_channel_logits = [[float(v) for v in tmp[j]] for tmp in channel_logits] if use_attention else []
            ...
            result_objs.append({'mention_id': sample.mention_id, 'labels': labels,
                                'logits': [float(v) for v in sample_logits], 'att': [float(v) for v in att_vec],
                                'channel_logits': cur_channel_logits})
            # if 'McClain' in sample.mention_str:
            #     idxs = np.argwhere(tmp_el_type_vecs[j] > 0)
            #     print(labels, tmp_el_sgns[j], [gres.type_vocab[idx] for idx in idxs.squeeze(axis=1)])
...
def __check_att_vecs(result_objs, n_types):
    n_att = 3
    att_type_vecs = np.zeros((n_att, n_types), np.float32)
    for r in result_objs:
        logits = np.array(r['logits'], np.float32)
        att_vec = np.array(r['att'], np.float32)
        # print(logits)
        logits = logits * (logits > 0)
        # print(logits)
        # exit()
        vals = np.matmul(np.reshape(logits, (-1, 1)), np.reshape(att_vec, (1, -1)))
        att_type_vecs += np.reshape(vals, (n_att, -1))
    print(att_type_vecs)
